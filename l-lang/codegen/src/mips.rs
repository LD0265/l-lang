...
impl Mips {
    ...
    pub fn generate(&mut self) -> String {
        self.compiled
            .push_str("# Assembly generated by lcc (l-lang compiler)\n\n");
    ...
    fn generate_function(&mut self, ir_function: IrFunction) {
        // seperate alloc instructions from everything else
        let (allocs, rest): (Vec<IrInstruction>, Vec<IrInstruction>) =
            ir_function.instructions.into_iter().partition(|i| {
                matches!(i, IrInstruction::Alloc { .. }) | matches!(i, IrInstruction::SaveRa)
            });
    ...
    fn generate_store_imm(&mut self, dest: IrReg, value: IrValue) {
        if let IrValue::Immediate(n) = value {
            self.emit_instruction("li", &format!("${}{}, {}", dest.prefix(), dest.value(), n));
    ...
    fn generate_store_stack(
        ...
        self.emit_instruction(
            ...
            &format!(
                ...
                src.prefix(),
                src.value(),
                allocator.get_variable_offset(&symbol)
            ),
        );
    }
